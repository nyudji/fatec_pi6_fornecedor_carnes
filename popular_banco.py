...
for nome, tipo, preco_venda in produtos:
    ...
    cursor.execute("""
        ...
        VALUES (%s, %s, %s, %s, %s, %s)
    """, (nome, tipo, "Kg", preco_compra, preco_venda, 1))
...
produtos_entrada = []
for i in range(5000):
    data = datetime.now() - timedelta(days=random.randint(1, 90))
    entradas.append((data.date(), 1))
...
for _ in range(50000):
    data_pedido = datetime.now().date() - timedelta(days=random.randint(1, 30))
    status = random.choices(
        ["Pendente", "Faturado", "Entregue", "Cancelado"],
        weights=[0.3, 0.3, 0.35, 0.05],  # Apenas 5% cancelados
        k=1
    )[0]
    pedidos.append((random.randint(1, 500), data_pedido, status, 0.0))
execute_values(cursor, """
...
for i, (pid, data) in enumerate(pagaveis):
    ...
    pagamentos.append((pid, data + timedelta(days=random.randint(1, 5)), f"LS2025{i+1:05d}", round(random.uniform(100, 5000), 2), random.choice(metodos), status))
execute_values(cursor, """
